...
def main() -> None:
    df = load_raw_data()
    split = make_classification_split(df, "cc50", threshold="median")
    print(f"Порог CC50 (медиана): {split['cutoff']:.4f}")
    ...
    results = {
        "task": "classification_cc50_median",
        "target": "CC50, mM",
    ...
    path = save_results("classification_cc50_median", results)
