# Регрессия для предсказания IC50.
...
def main() -> None:
    df = load_raw_data()
    split = make_regression_split(df, "ic50", log_transform=True)

    print("Подбор гиперпараметров для IC50 (log1p)...")
    ...
    path = save_results("regression_ic50", results)
