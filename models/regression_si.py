# Регрессия для предсказания SI (selectivity index).
...
def main() -> None:
    df = load_raw_data()
    split = make_regression_split(df, "si", log_transform=True)

    print("Подбор гиперпараметров для SI (log1p)...")
    ...
    path = save_results("regression_si", results)
