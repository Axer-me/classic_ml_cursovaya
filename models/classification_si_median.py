...
def main() -> None:
    df = load_raw_data()
    split = make_classification_split(df, "si", threshold="median")
    print(f"Порог SI (медиана): {split['cutoff']:.4f}")
    ...
    path = save_results("classification_si_median", results)
